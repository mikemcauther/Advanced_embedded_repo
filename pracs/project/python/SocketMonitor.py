...
class SocketMonitor():
    def __init__(self,message_board=None):
        ...
        self._thread = Thread(target=self.process, args=([self._outer_message_queue,self.sockets]))
    ...
    def process(self,queue,sockets):
        packet = ""
        while True:
            try:
                #event = queue.get()
                #packet = self.sockets.socket_recv()
                packet = sockets.socket_recv()
                #msg = map(ord,packet)
                event = packet.decode('utf-8')
                print(""+event)
                self.WriteMessage(event)
            except Empty:
